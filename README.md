...
- **画布角标**：右上角信号条图标实时显示传输档位（1-3 格 = 省流/均衡/流畅，自动调节），不遮挡操作。
...
| 实时直播 | 打开页面自动开启连续画面（常开无需管理），传输档位**按网络状况自动升降**；**画面没变化几乎不耗流量**，切到后台自动暂停。实时开启时点击/输入**秒回无蒙层**，操作反馈由画面直接呈现 |
...
| Node.js | 负责"传话"和"动手操作"（截图、点击、输入） | 必须 |

> 别被名字吓到。Node.js 是免费、正规、社区里常用的软件，安装时一路点"下一步"就好。
...
3. **第一次**先输入 `npm install` 回车，安装项目依赖（看到装完提示即可，之后不用再装）；
4. 输入 `npm start` 回车，看到"花妖操控台已启动"就成功了。

> 首次启动时 Windows 可能会弹出"是否允许此应用更改设备"的授权窗口，点「是」即可——这是为了给花妖放行防火墙，避免新版花妖启动时弹联网提示。

> 另外，想让"更新花妖版本"功能可用，还需要在项目文件夹里建一个 `.env` 文件并填上你的下载地址，详见文件夹里的 `.env.example`（模板）注释。
...
**Q：打开页面提示"花妖尚未安装"或"程序文件丢失"？**
A：说明花妖程序还没装到电脑上（或被移动、被杀毒软件误删了）。点画面中间的「立即下载安装」，输入版本号（重新安装可填原版本号），按面板指引完成即可，装好后画面会自动出现。首次安装前需先在 `.env` 里配置好下载源（见 `.env.example`）。
...
**Q：画面黑屏或内容很旧？**
A：实时画面会自动保持最新。若长时间未更新，多为网络波动导致重连中，稍候即可恢复；重开页面能立即看到最新画面。

**Q：点击了却没反应？**
A：多半是花妖窗口被最小化或缩到托盘了。恢复显示再试；实时画面恢复后会自动显示花妖当前的真实样子。
...
- 「实时直播」在蜂窝网络下建议用**省流档**且看完及时关闭；画面静止时几乎不耗流量，但持续动画场景下各档流量约为：省流 ~90MB/时、均衡 ~360MB/时、流畅 ~900MB/时。
